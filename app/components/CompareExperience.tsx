...
import { getRoamingResult, getScenarioOptions, networkNames, pricedRoamingCoverage, ROAMING_CHECKED_AT, type Network } from "@/lib/roaming";
...
export default function CompareExperience({ initial = defaultComparison, destinationLanding = false, livePlans, liveDestinationIds, heroAnswer, guide }: { initial?: InitialComparison; destinationLanding?: boolean; livePlans?: Plan[]; liveDestinationIds?: DestinationId[]; heroAnswer?: string; guide?: ReactNode }) {
  ...
  // Results are shown on load, so the live region must only speak after a submit.
  ...
  const destinationsByRegion = useMemo(() => {
    ...
    const order = ["Europe", "Europe & Asia", "Middle East", "Asia", "North America", "Africa", "Oceania"];
    const rank = (region: string) => (order.indexOf(region) === -1 ? order.length : order.indexOf(region));
  ...
  const groupedPlans = useMemo(() => {
    const compareMatches = (a: PlanMatch, b: PlanMatch) => {
      ...
      const aRankable = !a.catalogueOnly && a.gbpTotal !== null;
      const bRankable = !b.catalogueOnly && b.gbpTotal !== null;
    ...
    return (Object.keys(providerDetails) as Provider[]).map((provider) => {
      const matches = comparisonPlans
        ...
        .sort(compareMatches);
      const rankable = matches.filter((plan) => plan.gbpTotal !== null && !plan.catalogueOnly && (callsNeed !== "yes" || plan.callingSupport === "calls-texts"));
  ...
  const bestPricedPlan = [...allMatches]
    .filter(({ plan }) => plan.gbpTotal !== null && !plan.catalogueOnly && (callsNeed !== "yes" || plan.callingSupport === "calls-texts"))
  ...
  return (
    <>
      ...
      <main className="home-page" id="main-content">
        <div className="hero-stage">
          <section className="hero" id="top">
            <div className="hero-copy">
              ...
              <p className="hero-cta"><a href="#results">See the cheapest plans for {activeDestination.name} <span aria-hidden="true">↓</span></a></p>
            ...
            <form className="compare-card" id="compare" aria-labelledby="compare-title" onSubmit={(event) => { event.preventDefault(); compare(); }}>
              ...
              <button className="primary-button" type="submit">{hasCompared ? "Update my comparison" : "Compare my trip"} <span aria-hidden="true">→</span></button>
        ...
        <section className={`results-section ${hasCompared ? "is-visible" : ""} ${liveLoading ? "is-loading" : ""}`} id="results" aria-hidden={!hasCompared} aria-busy={liveLoading}>
          ...
          <div className="provider-list">{groupedPlans.map(({ provider, matches, dominatedIds }) => {
            ...
            return <article className={`provider-card ${bestPricedPlan?.provider === provider ? "top-pick" : ""}`} key={provider}><header className="provider-header"><div className="provider-main"><div className="provider-logo" style={{ background: details.accent }}>{details.initials}</div><div><div className="provider-name-row"><h3>{provider}</h3></div><p>{details.summary}</p></div></div></header><div className="plan-list">{matches.map((plan, planIndex) => {
              ...
              const showFitBadge = planIndex === 0 || requirementMismatch || callsUnverified;
          ...
          <p className="results-disclaimer">Plans marked “Live price” come straight from the provider’s own feed: Saily quotes real pounds, Nomad quotes US dollars we convert at rounded rates (€1 ≈ £{gbpRates.EUR}, $1 ≈ £{gbpRates.USD}), so treat converted totals as close estimates. Hand-checked prices carry the date we last confirmed them, in the currency shown. Klook prices are never scraped or guessed. We only show a saving when roaming covers your whole trip and everything else you ticked, and because we can’t confirm 5G roaming on any network, ticking “5G listed” hides savings entirely. Commission never changes the order. Always confirm price, coverage, hotspot rules, speed caps, fair use, calls and texts at checkout.</p>
        </section>
        ...
        <section className="methodology-section" id="methodology"><div className="methodology-intro"><p className="eyebrow">Where the numbers come from</p><h2>How we work this out.</h2><p>{liveDestinationCount > 0 ? `Every roaming figure comes from a charge the network publishes: EE for all ${destinations.length} destinations, plus ${pricedRoamingCoverage().others} more network-and-country pairings across the other nine. Where a network publishes no rate, we send you to its own checker rather than guess. On the eSIM side, Saily and Nomad send us live prices for ${liveDestinationCount} destinations, Airalo's are checked by hand for ${pricedDestinationIds.length}, and Klook shows its prices only on its own page.` : `Every roaming figure comes from a charge the network publishes: EE for all ${destinations.length} destinations, plus ${pricedRoamingCoverage().others} more network-and-country pairings across the other nine. Where a network publishes no rate, we send you to its own checker rather than guess. On the eSIM side, ${pricedDestinationIds.length} destinations have prices we checked by hand; for the other ${destinations.length - pricedDestinationIds.length} we send you to the provider rather than show a number we cannot stand behind.`}</p><div className="data-status"><span><i className="fresh" />UK roaming rules <strong>Checked {formatCheckedDate(ROAMING_CHECKED_AT)}</strong></span><span><i className="fresh" />Hand-checked prices &amp; calls/texts labels <strong>Checked {formatCheckedDate(DATA_CHECKED_AT)}</strong></span><span><i className="fresh" />Exchange rates (rounded) <strong>Checked {formatCheckedDate(FX_EVIDENCE.checkedAt)}</strong></span><span><i className="manual" />Price refresh rule <strong>Rechecked on a weekly schedule</strong></span><span><i className={liveDestinationCount > 0 ? "fresh" : "manual"} />Live prices from Saily and Nomad <strong>{liveDestinationCount > 0 ? `working · ${liveDestinationCount} destinations` : "not answering — showing hand-checked prices"}</strong></span></div></div><details className="source-register"><summary>Sources for {activeDestination.name} <span>Every price’s paper trail</span></summary><div className="source-grid">{currentSources.length > 0 ? currentSources.map((plan) => <a href={plan.sourceUrl} target="_blank" rel="noopener noreferrer" key={plan.sourceUrl}><span>Where we checked</span><strong>{plan.provider} {activeDestination.name}</strong><small>{plan.note} · checked {formatCheckedDate(plan.checkedAt)} · review by {formatCheckedDate(plan.reviewAfter)}</small></a>) : (Object.keys(providerDetails) as Provider[]).map((provider) => <a href={getProviderSourceUrl(provider, activeDestination)} target="_blank" rel="noopener noreferrer" key={provider}><span>Where to check</span><strong>{provider} {activeDestination.name}</strong><small>Price and exact limits are shown on their site</small></a>)}{roaming && <a href={roaming.evidence.url} target="_blank" rel="noopener noreferrer"><span>UK roaming</span><strong>{roaming.evidence.label}</strong><small>Checked {formatCheckedDate(roaming.evidence.checkedAt)} · review by {formatCheckedDate(roaming.evidence.reviewAfter)}</small></a>}<a href={FX_EVIDENCE.url} target="_blank" rel="noopener noreferrer"><span>Currency method</span><strong>{FX_EVIDENCE.label}</strong><small>Rounded comparison assumptions · checked {formatCheckedDate(FX_EVIDENCE.checkedAt)} · review by {formatCheckedDate(FX_EVIDENCE.reviewAfter)}</small></a></div></details></section>
