...
import { CALLS_CHECKED, DATA_CHECKED, plans, providerDetails, tripLengths, usagePerDay, type Plan, type Provider, type Usage } from "@/lib/catalog";
  ...
  return (
    <>
      ...
      <section className={`results-section ${hasCompared ? "is-visible" : ""}`} id="results">
        ...
        {bestPricedPlan && destination === "turkey" && <div className="decision-card"><div><span className="decision-label">Best priced data fit</span><h3>{bestPricedPlan.provider} · {bestPricedPlan.plan.name}</h3><p>Enough data for this trip based on your selected usage. Calls and texts are not part of this ranking, so compare the live checkout against your network total before buying.</p></div><div className="decision-price"><small>Snapshot total</small><strong>≈ {money.format(bestPricedPlan.plan.gbpTotal!)}</strong>{canCompareSavings && roaming.cost !== null && roaming.cost - bestPricedPlan.plan.gbpTotal! > 0.5 && <span>About {money.format(roaming.cost - bestPricedPlan.plan.gbpTotal!)} less than roaming</span>}</div><a href={getProviderUrl(bestPricedPlan.provider, activeDestination)} target="_blank" rel={providerDetails[bestPricedPlan.provider].affiliate ? "sponsored noopener noreferrer" : "noopener noreferrer"}>Check live price <span aria-hidden="true">↗</span></a></div>}
        ...
        <aside className={`calling-guide ${destination === "turkey" ? "data-only" : "check-plan"}`}>
          <span aria-hidden="true">☎</span><div><small>Calls and texts</small><strong>{destination === "turkey" ? "Every priced eSIM shown here is data-only" : "Calls and texts vary by provider plan"}</strong><p>{destination === "turkey" ? "WhatsApp, FaceTime and similar apps can call using the eSIM’s data. Standard phone calls and SMS need your UK line or a separate phone-number plan; Saily’s optional call-and-text plan is not included in these displayed prices." : "These are live catalogue links, so check whether the exact plan says Data only or includes a phone number, calls and texts before buying."} <a href="#methodology">Sources checked {CALLS_CHECKED}</a>.</p></div>
        </aside>
                  ...
                  {matches.map((plan, planIndex) => {
                    ...
                    return (
                      <div className="plan-row" key={plan.id}>
                        <div className="plan-copy"><div><strong>{plan.name}</strong>{planIndex === 0 && <span className="fit-badge">{plan.catalogueOnly ? "Live provider search" : "Lowest suitable snapshot"}</span>}</div><p>{plan.catalogueOnly ? `Browse current ${activeDestination.name} allowances and durations` : `${plan.unlimited ? "Unlimited data" : plan.dailyDataGb ? `${plan.dailyDataGb}GB high-speed data each day` : `${plan.suppliedData}GB supplied`} · ${plan.speed}`}</p><div className={`calls-texts-status ${plan.callingSupport}`} data-status={plan.callingSupport}><strong>{plan.callingSupport === "data-only" ? "Data only" : plan.callingSupport === "calls-texts" ? "Calls & texts included" : "Calls & texts: check plan"}</strong><span>{plan.callingSupport === "data-only" ? provider === "Saily" ? "App calls work over data; Saily’s optional phone-number plan costs extra." : "App calls work over data; no phone number or standard calls/SMS." : plan.callingSupport === "calls-texts" ? "A phone number and standard mobile service are included; confirm the allowance." : "Plan types can vary in the live catalogue."}</span></div><small>{plan.catalogueOnly ? plan.note : `${plan.packs > 1 ? `${plan.packs} packs estimated for ${days} days` : `${plan.validity}-day validity`} · ${plan.network} · ${plan.note}`}</small></div>
      ...
      <section className="methodology-section" id="methodology">
        ...
        <details className="source-register"><summary>View the full source register <span>18 receipts</span></summary><div className="source-grid">
          ...
          <a href="https://www.airalo.com/help/using-managing-esims/ZSEEHBT5HW6F/how-can-i-use-calls-and-texts-with-my-esim/3CR1PNGN426T" target="_blank" rel="noopener noreferrer"><span>Calls & texts</span><strong>Airalo plan types</strong><small>Turkey plans: data only · checked 16 Aug 2026</small></a>
          <a href="https://www.nomadesim.com/help-center/en/articles/9886329-do-nomad-esims-come-with-a-phone-number" target="_blank" rel="noopener noreferrer"><span>Calls & texts</span><strong>Nomad phone numbers</strong><small>Turkey plans: data only · checked 16 Aug 2026</small></a>
          <a href="https://saily.com/esim-phone-number/" target="_blank" rel="noopener noreferrer"><span>Calls & texts</span><strong>Saily phone-number plans</strong><small>Separate from displayed data prices · checked 16 Aug 2026</small></a>
          <a href="https://www.klook.com/en-GB/activity/128551-turkey-esim-high-speed-internet-qr-code-voucher/" target="_blank" rel="noopener noreferrer"><span>Calls & texts</span><strong>Klook Turkey service type</strong><small>Data only · checked 16 Aug 2026</small></a>
        </div></details>
      </section>
      ...
      <section className="faq-section" id="faq">
        ...
        <div className="faq-list">
          ...
          <details><summary>Can I make calls with these eSIMs?</summary><p>The priced Turkey eSIMs shown here are data-only. WhatsApp, FaceTime and similar apps can call over their data, but normal mobile calls and SMS are not included. You can keep your UK line active for those services—check roaming charges—or buy a separate phone-number plan where available.</p></details>
